/* Main app shell: sidebar, topbar, screens, drawer, modal, theme toggle, tweaks */
...
function App() {
  ...
  // Search + filter (lifted; shared by Accounts screen)
  const [search, setSearch] = useState("");
  const [activeCat, setActiveCat] = useState("all");

  // Drawer + modal state
  const [drawer, setDrawer] = useState(null); // { kind: 'txn'|'holding'|'account', data }
  const [modal, setModal] = useState(null);   // { kind: 'add'|'switch' }
  ...
  const navItems = [
    ...
    { id: "accounts", label: "Accounts", icon: "wallet", count: window.DATA.transactions.length },
    { id: "cards", label: "Credit Cards", icon: "card", count: window.DATA.accounts.filter(a => a.kind === "credit").length },
    ...
    <>
      ...
      <div className="app">
        ...
        <aside className="card sidebar">
          ...
          {navItems.map(n => (
            <div key={n.id} className={`navitem ${tab === n.id ? "active" : ""}`}
                 onClick={() => setTab(n.id)}>
        ...
        <header className="topbar">
          <div className="crumb"><span style={{ opacity: 0.6 }}>{crumb[0]}</span> <span style={{ margin: "0 8px", color: "var(--text-3)" }}>/</span> <b>{crumb[1]}</b></div>
          <div className="search">
            <Icon name="search" size={14} />
            <input value={search} onChange={e => { setSearch(e.target.value); if (e.target.value && tab !== "accounts") setTab("accounts"); }}
        ...
        <main className="main">
          ...
          {tab === "cards" && (
            <CardsScreen onOpenTxn={openTxn} />
          )}
          {tab === "investments" && (
            <InvestmentsScreen onOpenHolding={openHolding}
                               range={range} setRange={setRange} />
          )}
        </main>
  ...
  return (
    <div style={{ position: "absolute", bottom: "calc(100% + 6px)", left: 0, right: 0,
      ...
      {[
        ...
        <div key={p.name} className="navitem" onClick={onClose}

/* Credit Cards module — per-card consumption analytics */
...
// Filter txns by range, scoped to a given account list
...
function CardsScreen({ onOpenTxn }) {
  const d = window.DATA;
  const creditCards = d.accounts.filter(a => a.kind === "credit");
  const [activeId, setActiveId] = React.useState(creditCards[0].id);
  ...
  // Filter card transactions by selected range
  ...
  // Trend series for this card — last 6 months absolute spend
  ...
  // Recurring detection — same merchant ≥2 times in last 90d
  ...
  return (
    <div className="screen">
        ...
        <div className="span-5">
          ...
          <div className="card" style={{ marginTop: 14, padding: 8 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
              <CardCtrl icon="pin" label={frozen ? "Frozen" : "Freeze"} active={frozen}
                onClick={() => setFrozen(f => !f)} />
              <CardCtrl icon="card" label={showNumber ? "Hide" : "Show"}
      ...
      <div className="grid cols-4">
        <KPITile label={`Total spend · ${rangeLabel}`} value={fmt(totalSpend)} mono
                 sub={`${charges.length} charges`} />
        ...
        <KPITile label="Unique merchants" value={uniqueMerchants.toString()} mono
        ...
        <KPITile label={card.rewards.kind === "miles" ? "Miles earned" : "Cashback earned"}
                 ...
                 mono accent
                 sub={`+${card.rewards.monthly.toLocaleString()} this month`} />
      </div>
        ...
        <div className="span-8">
          <Card>
            ...
            <div className="cardbody">
              <AreaChart
                labels={sixMo}
                series={[{ name: "Balance", data: trendSpend, primary: true }]}
                height={200}
              />
            </div>
          </Card>
      ...
      {/* Analytics: category · merchant · subcategory */}
      <div className="grid cols-12">
        <div className="span-4">
          <Card>
            <div className="cardhead">
            ...
            <div className="cardbody" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
              {byCategory.length > 0 ? <Donut data={byCategory.map(c => ({ label: c.label, value: c.value }))} size={160} thickness={18} />
        ...
        <div className="span-4">
          <Card>
            ...
            <div className="cardbody" style={{ paddingTop: 8 }}>
              {byMerchant.length > 0 ? (
                <TopBarList items={byMerchant.map(m => ({
                  label: m.label, value: m.value, count: m.count, avg: m.value / m.count,
                  secondary: m.lastDate
                }))} limit={8} />
        ...
        <div className="span-4">
          <Card>
            ...
            <div className="cardbody" style={{ paddingTop: 8 }}>
              {bySubcat.length > 0 ? (
                <TopBarList items={bySubcat.map(s => ({
                  label: s.label, value: s.value, count: s.count,
                  secondary: s.parentCat,
                }))} limit={8} />
...
/* ============ Helpers (recurring detection) ============ */
function detectRecurring(txns) {
  // Group by merchant; if count ≥ 2 and average gap between dates is 25-35 days → monthly
  const byMerchant = {};
  for (const t of txns) {

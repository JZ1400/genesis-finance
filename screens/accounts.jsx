/* Accounts screen — hero balance chart + filters + deep analytics */
...
function AccountsScreen({ onOpenTxn, onAddTxn, search, setSearch, activeCat, setActiveCat }) {
  const d = window.DATA;
  const [activeAcct, setActiveAcct] = React.useState("all");
  const [range, setRange] = React.useState("6m");

  // Compute series for hero chart
  ...
  // Filter transactions by selected account + range
  ...
  return (
    <div className="screen">
      ...
      {/* HERO: total accumulated balance chart */}
      <Card thick>
        ...
        <div style={{ padding: "20px 12px 12px" }}>
          <AreaChart
            labels={labels}
            series={[{ name: "Balance", data: heroSeries, primary: true }]}
            height={260}
          />
        </div>
      </Card>
      ...
      <div className="card" style={{ padding: 12, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {cats.map(c => (
            <button key={c.id} className={`chip ${activeCat === c.id ? "active" : ""}`}
            ...
            <div className="cardbody" style={{ paddingTop: 8 }}>
              {bySubcat.length > 0 ? (
                <TopBarList items={bySubcat.map(s => ({
                  label: s.label, value: s.value, count: s.count, secondary: s.parentCat,
                }))} limit={8} />

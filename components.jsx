// Shared icons, chart primitives, helpers
// Exposes globals via window.* for cross-file babel scripts.
...
/* ============ Card ============ */
const Card = ({ children, className = "", title, eyebrow, right, padding = true, thick = false, style }) => (
...
/* ============ Delta (monochrome, arrow-only) ============ */
const Delta = ({ value, format = "pct", showSign = true }) => {
  const cls = value > 0.0005 ? "up" : value < -0.0005 ? "dn" : "flat";
  const text = format === "pct"
    ? fmtPct(value, { sign: showSign })
    : (value > 0 ? "+" : "") + fmt(value);
  return <span className={`delta ${cls}`}>{text}</span>;
};

/* ============ Sparkline ============ */
const Sparkline = ({ data, width = 96, height = 28, stroke = 1.5, filled = true }) => {
...
const AreaChart = ({ labels, series, height = 220, formatY = (v) => fmt(v, { compact: true }) }) => {
  // series: [{name, data, dashed?, primary?}]
...
/* ============ Bar chart (paired bars: this vs prev) ============ */
const BarPairs = ({ rows, height = 240, max }) => {
  const wrapRef = useRef(null);
  const [w, setW] = useState(640);
        ...
        {rows.map((r, i) => {
          ...
          return (
            <g key={r.label} onMouseEnter={() => setHover({ i, r, x: cx, y: yAt(Math.max(r.a, r.b)) })}
              ...
              {/* prev (lighter / outlined) */}
              <rect x={ax} y={yAt(r.b)} width={barW} height={innerH - (yAt(r.b) - padT)}
                    fill="var(--line-2)" rx="2" />
              {/* this (solid) */}
              <rect x={bx} y={yAt(r.a)} width={barW} height={innerH - (yAt(r.a) - padT)}
...
/* ============ Donut (monochrome — uses fills by index) ============ */
const Donut = ({ data, size = 180, thickness = 22 }) => {
  ...
  let acc = 0;
  // Monochrome fills: vary alpha across slices.
  const tone = (i, n) => `rgba(var(--mono-rgb, 255 255 255), ${0.95 - (i / Math.max(1, n - 1)) * 0.6})`;
  // We'll use real colors via CSS vars instead — define tone via inline:
  // Ramp through accent + neutral mix
  ...
  return (
    <div style={{ display: "flex", width: "100%", height, borderRadius: 4, overflow: "hidden", border: "1px solid var(--line-1)" }}>
      {data.map((d, i) => {
        ...
        const base = i % 2 === 0 ? "var(--accent)" : "var(--text-1)";
        return (
          <div key={d.label} title={`${d.label} · ${fmtPct(d.value / total, { dec: 0 })}`}
            style={{
...
/* ============ Progress (used for budget) ============ */
const Progress = ({ value, max, height = 5 }) => {
...
/* ============ Top bar list (merchants / subcategories) ============ */
const TopBarList = ({ items, limit = 8, valueFmt = (v) => fmt(v, { compact: true }), onClick }) => {
  // items: [{ label, value, count?, secondary? }]
...
/* ============ Range picker ============ */
const RangePicker = ({ value, onChange, options }) => (
...
/* ============ Aggregation helper (used by analytics screens) ============ */
const aggregate = (txns, keyFn, labelFn) => {
  const map = {};
  for (const t of txns) {
    const k = keyFn(t);
    ...
    map[k].count += 1;
    if (!map[k].lastDate || t.date > map[k].lastDate) map[k].lastDate = window.fmtDate(t.date);
  }
...
Object.assign(window, {
  Icon, Card, Delta, Sparkline, AreaChart, BarPairs, Donut, StackBar, Progress,
  TopBarList, RangePicker, aggregate,
  CAT_META,
});
